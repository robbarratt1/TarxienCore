...
public class TarxienCore : MonoBehaviour
{
    ...
    private void Start()
    {
        ...
        // Sets up default scene, based on Mnajdra South.
    ...
    void DrawLine()
    {
        // Draws a line in the editor between the astronomical body and the far end of the temple.
        if (isSaving == false)
        {
            if (activeAstroList.Count == 0)
            {
                Debug.DrawRay(activeAstro.transform.position, target.transform.position - activeAstro.transform.position, Color.red);
            }
    ...
    void UpdateDisplay()
    {
        // Updates the date and time displayed on the main UI.
        if (isSaving == false)
        {
            string tempYear = "";
            if (actualYear < 2000)
    ...
    void ChooseAstro()
    {
        // Sets the astronomical body based on the selection in the UI.
    ...
    void CreateObjects()
    {
        // The stars are organised into constellation lists.
    ...
    void Calculations()
    {
        // Calculates basic elements applicable to all astronomical bodies. 
        jde = CalculateJD();
        CalculateDay();
        CalculateEarthAxis();
        SiteHeight();
        ...
        // Initiate calculations for the position of the sun, moon or stars.
        if (activeAstro == sun)
        {
            SunCalculations();
        }
    ...
    void ParseFields()
    {
        // Obtains the longitude, latitude, horizon and orientation based on the Input Fields.
        double number;
    ...
    void CalculateDay()
    {
        // Obtain day of the year based on month.
        if (month.value == 1)
        {
            dayOfYear = day.value;
        } else if (month.value == 2)
    ...
    {
        // Obtain declination of Earth's axis at given time.
    ...
    double CalculateJD()
    {
        // Calculate the Julian Day for the date inputted.
        ...
        else if ((2000 - (year.value)) < 1582)
        {
            ...
            // Fix leap years prior to 1582 AD.
    ...
    {
        // Add nutation to the Earth's axis.
    ...
    void SiteHeight()
    {
        // Obtain site height from the Input Field.
    ...
    {
        // Reorient the temple, based on the position of the entrance and back. Before running
        // the script, position the four spheres at eather sides of the entrance and of the back niche.
    ...
    {
        // Elevates the temple based on the site height, with scaling applied based on the distance
        // from the astronomical body.
        if (scalingFactor > 0)
        {
    ...
    void UpdateSiteName()
    {
        // Updates the name of the site for the CSV file.
    ...
    {
        // Calculate the sun position at given time.
        double jDate = (jde - 2451545) / 36525;

        // Obtain the apparent ascension and declination.
        apparentAscension = CalculateAscension(jDate);
        apparentDeclination = CalculateDeclination(jDate);

        // Obtain the Greenwich time, the mean longitude, equation of time and hour angle.
        ...
        // Adjust for Unity coordinates.
        double tempAz = azimuth + 180;
        azimuth = -(tempAz + 90);
        
        // Adjust sun distance by scaling factor.
        ...
        // Calculate cartesian coordinates of the sun and apply to GameObject.
        Vector3 pos = SphericalToCartesian(earthToSun, azimuth, altitude);
        sun.transform.position = pos;

        // Calculate solstices and equinox.
        CheckSolEqui(apparentLongitude);
    }

    double Normalise(double input)
    {
        // Normalise an angle so the range is between 0 and 360.
    ...
    double CalculateRefraction(double alt)
    {
        ...
        return alt + (1.02f / Mathf.Rad2Deg * (Math.Tan(Mathf.Deg2Rad * (altitude + (10.3 / (altitude + 5.11))))));
    }

    void CalculateSunData(double date)
    {
        // Obtain basic solar data, such as mean, true and apparent longitude, mean and true anomaly,
        // eccentricity of the axis, centre of the sun, radius and omega.
        double t = date;
    ...
    {
        // Find the date of the closet solstices and equinoxes, by calculating the times with apparent longitude
        // closest to 0, 90, 180, 270 and 360 degrees.
    ...
    {
        // Calculate right ascension at a given time.
    ...
    {
        // Calculate declination at a given time.
        CalculateSunData(date);
    ...
    {
        // Calculate moon position, based on longitude, elongation, mean anomaly, anomaly and argument of latitude.
        ...
        // Calculate the L, R and B by summing the lunar terms based on date.
        ...
        // Obtain positive angle.
        if (moonRightAscension < 0) {
            moonRightAscension += 360;
        }

        // Convert ascension and declination to azimuth and altitude.
        ...
        // Find closest lunar stand stills to the date.
        ClosestDec();
    }

    void CreateMoonTerms()
    {
        // Terms used for the calculation of L, R and B. Due to gravitational pull of other bodies,
    ...
    void SphereAssoc()
    {
        // Create a sphere in Unity3D and associate it to the cosmological body.
        if (isSaving == false)
        {
    ...
    void DeleteSpheres()
    {
        // Remove associated spheres if no longer in use.
        if (isSaving == false)
        {
            foreach (GameObject sp in spheres)
            {
    ...
    void ClosestDec()
    {
        // Obtain the date for the lunar stand stills based on the date. Find the ascending node and
        // calculate the  dates when it is closest to 0, 180 and 360 degrees.
        if (tempNum != year.value)
        {
            ...
            for (int tempJde = (int)Math.Floor(jde - 3652.5); tempJde < jde + 3652.5; tempJde++)
            {
                ...
                double ascNode = Normalise(125.0445550 - 1934.1361849 * t + 0.00020762 * Math.Pow(t, 2) + Math.Pow(t, 3) / 467410 +
    ...
    {
        // Set the position and proper motion of stars.
    ...
    void CalculateStar()
    {
        ...
        // Determine whether it is a single star or a constellation.
        ...
        // Find right ascension and proper motion based on JD2000 and proper motion.
        ...
        // Obtain corrected right ascension and declination.
        ...
        // Transform ascension and declination into azimuth and altitude.
        double greenTime = Normalise(280.46061837 + 360.98564736629 * (jde - 2451545) + 0.000387933 * Math.Pow(t, 2) -
            Math.Pow(t, 3) / 38710000 + deltaPsi * Math.Cos(Mathf.Deg2Rad * trueAxis));
        ...
        // Transform spherican to cartesian coordinates.
    ...
    void CalculateConst()
    {
        // Calculate the position of a constellation.
        ...
        {
            // Calculate single star's position.

            // Obtain right ascension and declination based on JD2000 and proper motion.
    ...
    void ButtonPressed()
    {
        // When the button is pressed. the saving process begins.
        isSaving = true;

        // Create titles of CSV file.
        rowData.Clear();
        jdeList.Clear();
        double firstAlig = 0;
        ...
        // Loop the date for every minute of the year.
        if (activeAstroList.Count == 0)
        {
            for (int a = 1; a < 13; a++)
            {
                ...
                for (int b = 1; b < days; b++)
                {
                    for (int c = 0; c < 24; c++)
                    {
                        for (int d = 0; d < 60; d++)
                        {
                            ...
                            // Calculate the position of the selected cosmological body.
                            Calculations();

                            // Calculate horizon.
                            double hor = CalculateTargetHorizon(absoluteAltitude, absoluteDistance);

                            // Cast a ray between the body and the back of the temple.
                                ...
                                Vector3.Distance(activeAstro.transform.position, target.transform.position)))
                            {
                                ...
                                {
                                    // Add a new line to the CSV file, with the start of the alignment.
                                    if (firstAlig == 0)
                                    {
                                        if (hor > 0)
                                        {
                                    ...
                                    // Add a new line to the CSV file, with the end of the alignment.
        ...
        {
            // Run the same loop for constellations.
            int absCount = 0;
        ...
        // Add extra information and save the CSV file.
        AddExtras();
        SaveDocument();
        isSaving = false;
    }
    ...
    {
        // Add a new line to the CSV file with the alignment information.
        string[] rowDataTemp = new string[12];
        rowDataTemp[0] = name;
    ...
    void SaveDocument()
    {
        // Save the lines to the CSV files, based on a script by Sushanta (2015).
    ...
    void AddExtras()
    {
        // Add information such as equinox, solstice and lunar stand still dates.
        if (activeAstro == sun)
        {
    ...
    {
        // Calculate horizon of the temple.
    ...
    void QuickSave()
    {
        // Calculate the alignment of the temples for all cosmological bodies between 3600 and 2100 BC.
        ...
        // Calculate the alignment of the temples for selected cosmological body between 3600 and 2100 BC.
        if (Input.GetKeyDown(KeyCode.Return))
        {
            year.value = 5600;
            AssignSiteHeight();
    ...
    void YearDecide()
    {
        // Correct the year in the text UI.
        string tempYear = "";
        if (year.value < 2000)
        {
            tempYear = (2000 - year.value).ToString() + " AD";
...
// MasterList class
#region
// Used to contain the information for stellar position calculation.
[System.Serializable]
